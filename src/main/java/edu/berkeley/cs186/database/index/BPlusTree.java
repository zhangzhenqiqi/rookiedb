...
/**
 * 这颗B+经过简化，而且它内部节点中有key不代表叶子中一定也有这个key，他是这样设计的。删除时也不必维护平衡，后期考虑加上。
 * 这颗B+树不允许出现重复的key。
 * A persistent B+ tree.
 * <p>
...
public class BPlusTree {
    ...
    /**
     * 返回key对应的记录id，如果不存在这个key返回Optional.empty()
     * <p> </p>
    ...
    /**
     * 当put、bulkLoad返回split-node时，说明root也分裂了，需要新建root节点，newRoot有两个儿子，
...
//        LockUtil.ensureSufficientLockHeld(lockContext, LockType.X);
        ...
        children.add(splitInfo.getSecond());
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> splitInfoOpt = root.put(key, rid);
        if (splitInfoOpt.isPresent()) {
            Pair<DataBox, Long> splitInfo = splitInfoOpt.get();
            splitRoot(splitInfo);
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        if (scanAll().hasNext()) {
            throw new RuntimeException("bulk load must be called in an empty tree.");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> splitInfoOpt = root.bulkLoad(data, fillFactor);
            if (splitInfoOpt.isPresent()) {
                Pair<DataBox, Long> splitInfo = splitInfoOpt.get();
                splitRoot(splitInfo);
            }
    ...
    /**
     * 由于这是简化后的B+树，可能出现删除完了，剩下个空节点，所以有节点不代表一定有值
     */
    private class BPlusTreeIterator implements Iterator<RecordId> {
        // TODO(proj2): Add whatever fields and constructors you want here.

        int cursor;//当前的叶子节点中的下一个即将访问的下标
        Optional<LeafNode> curNodeOpt;//当前遍历到的叶子节点
        ...
        BPlusTreeIterator() {
            ...
            if (leafNode == null) {
                curNodeOpt = Optional.empty();
            } else {
                curNodeOpt = Optional.of(leafNode);
            }
        }
        ...
        BPlusTreeIterator(DataBox key) {
            LeafNode leafNode = root.get(key);
            curNodeOpt = leafNode == null ? Optional.empty() : Optional.of(leafNode);
            List<DataBox> keys = curNodeOpt.get().getKeys();
            for (cursor = 0; keys.get(cursor).compareTo(key) < 0; cursor++) ;
        }

    void moveCursor() {
        for (; curNodeOpt.isPresent(); ) {
            LeafNode curNode = curNodeOpt.get();
            if (curNode.getKeys().size() == 0 || cursor == curNode.getKeys().size()) {
                cursor = 0;
                curNodeOpt = curNode.getRightSibling();
            } else break;
        }
    }
    ...
    public RecordId next() {
        ...
        return curNodeOpt.get().getRids().get(cursor++);
    }
}
}
